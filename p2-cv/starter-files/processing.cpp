...
void compute_energy_matrix(const Image *img, Matrix *energy) {
    ...
    int energy_of_x = squared_difference(N, S) + squared_difference(W, E);
            *Matrix_at(energy, row, col) = energy_of_x;
        }
    }
...
void compute_vertical_cost_matrix(const Matrix *energy, Matrix *cost) {
    int new_width = Matrix_width(energy);
    int new_height = Matrix_height(energy);
    Matrix_init(cost, new_width, new_height);
    Matrix_fill(cost, 0);

    for (int col = 0; col < new_width; col++) {
        *Matrix_at(cost, 0, col) = *Matrix_at(energy, 0, col);
    }
    for (int row = 1; row < new_height; row++) {
        for (int col = 0; col < new_width; col++) {
            if (col == 0) {
                *Matrix_at(cost, row, col) =
     *Matrix_at(energy, row, col) +
    Matrix_min_value_in_row(cost, (row - 1), (col), (col + 2));
            } else if (col == (new_width - 1)) {
        *Matrix_at(cost, row, col) =
     *Matrix_at(energy, row, col) +
...
void find_minimal_vertical_seam(const Matrix *cost, int seam[]) {
    ...
    int current_col = Matrix_column_of_min_value_in_row(cost, (height - 1), 
    (0), (width));
    // pick the min value of th very bottow row
    for (int row = height - 1; row > 0; row--) {
        seam[row] = current_col;
        if (current_col == 0) {
current_col = Matrix_column_of_min_value_in_row(cost, (row - 1), 
...
(current_col - 1), (current_col + 1));
    ...
    // when the row is 0 so no longer loops through the for loop you want
    // to set that seam value to the current_col from row 1 becuase
    // that is the one 0 will use to get to 1
    seam[0] = current_col;
}
...
void remove_vertical_seam(Image *img, const int seam[]) {
    ...
    int height = Image_height(img);
    Image_init(imgCopy, width, height);
    for (int row = 0; row < height; row++) {
        int current_col = seam[row];
        for (int col = 0; col < seam[row]; col++) {
            Image_set_pixel(imgCopy, row, col, 
            Image_get_pixel(img, row, col));
        }
        for (int col = current_col; col < width; col++) {
            Image_set_pixel(imgCopy, row, col, Image_get_pixel(img,
             row, (col + 1)));
        }
    }
    *img = *imgCopy;
